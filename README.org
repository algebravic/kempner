...
$P_a(s) = \sum_{n=-\infty}^\infty c_{a,n}  \sum_{p \ge 10^m,
\text{ prime}} \frac{1}{p^{s - (2 \pi n)/\log 10)i}.$$
Thus, since $g_a$ is real valued, we have $\overline{c_{a,n}} =
c_{a,-n}$ for all $n$. Thus
$P_a(s)  =c_{a,0} (P(s) - I_m(s)) + \sum_{n=1}^\infty \Re(c_{a,n} (P(s - (2
\pi n/\log 10) i) - I_m(s - (2 \pi n/\log 10)))).$$
